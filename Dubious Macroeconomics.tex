...
%Misshandling of import leakages - a major flaw in the macroeconomics of open economies}
%\title{\bf  Neglecting the import content of shifts in final demand  - a major flaw in current macroeconmic analysis of open economies}
\author{Nikolaus K.A. Läufer\\
...
demand management consisted primarily of public works programs and
...
an international economy and in particular in a globalized world,
...
%policy measure intended to stimulate the German econmy by granting a
...
mechanically and impliciitly equal to zero. From this malpractice a
...
%personal abberation but is a common theoretical error in the way
%macroeconomics is currently taught and practiced. By implication,
%all textbooks in macroecnomics for open economies need to be
...
Lets start by considering a simple closed economy macroeconomic
model:
\begin{equation}
 Y = C(Y-T) + I + A
\end{equation}
where Y = income, C = endogenous consumption, T= exogenous taxes, I
= private (exogenous) investement, A = exogenous public
...
taxes and publlic expenditures are treated as exogenous, while
imports depend, like consumption, on disposable income.
\begin{equation}
 Y = C(Y-T) + I + A + Ex - Im(Y-T).
\end{equation}
A change in exogenous public expenditures alone will lead to change
...
represented by $\alpha$ then the true change in equlibrium income
would be:
\begin{equation}
...
production which is equal to the marginal propenstiy to consume
...
parameters for import content being convieniently representable by
...
raised can turn into a gross analytical error. The error comitted
results in an overestimate of the expansion. An error of size
\begin{equation}
\alpha A
\end{equation}
is multplied by the multiplier
\begin{equation}
\frac{1}{1-c_d}.
\end{equation}
...
The error comitted can be expressed in two alternative ways. So far
...
Formally, Machlups procedure can be illustrated by comparing the
...
Machlups simplifying assumption amounts to using the last
...
``approximation`` would amount to a small error.  Machlups work is
...
undermine the defense of a specification error by reference to its
...
for Machlups ``simplifying`` assumption, his behaviour does not look
different from the one you would expect to see from someone whose
intention is to ``maximize`` the multiplier. We cannot exclude for
...
generalized Machlup´s theoretical approach without making or letting
...
of Leontiefs input-output analysis in that history. It is hard to
...
attention of such emminent authors like James E. Meade (1951) and
others.


\section{Defending the current practice fails}
In defense of the current practice of neglecting import content
...
I am also able to show why this line of defense does not solve the
problem.


Suppose, additional public expenditures consist entirely of imported
goods, then our corrected analyis, using equation (\ref{eq:e1})
...
Another line of defense of the current practice of neglecting import
content paramters might be to argue that a proper choice of the
...
At this point one might ask the "Who is willing to work with
...
parameters for the ecnsuing effects, especially when this flow
...
% \put(1.5,-5.0){\includegraphics[scale=1.0]{import-1.eps}}
...
by current taxes, public debt or by additonal money. Restrictions of
...
not to be equal to 1 in oder to obtain a zero public expenditure
...
converges to zero as the import content parameter of additonal
...
for unpleasent surprises in cases where the parameter is neglected,
...
way is likely to deemphasize the relevance of the problem for
...
to be revised and that all open economcy macroeconomic analyses that
...
The specification error committed is not neglectable because it is
...
T. Mettitt Brown, Specification and Uses of Econometric Models,
Macmillan, 1970 \\
...
Nikolaus K.A. Läufer, Haavelmo's theorem in open economies: the
insulation effect, unpublished manuscript, May 2009 \\
Nikolaus K.A. Läufer, Undermining the case for Keynesian Demand
...
National Income Multiplier, Blakiston Co., Philadelphia 1943, \\
 ...
 Erich Schneider, Einführung in die Wirtschaftstheorie, III. Teil,
 Geld, Kredit, Volkseinkommen und Beschäftigung, 4. verbesserte u.
